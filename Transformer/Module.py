...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, return_weight=False):
        batch_size = Q.size(0)
    ...
    def cal_one_vector(self, vector, memory, memory_new, i, return_weight=False):
        ...
        outputs, weight = self.ScaledDotProductAttention(query,
                                                         memory_new[:, i, ..., 0],
                                                         memory_new[:, i, ..., 1])
        outputs = outputs.transpose(1, 2).contiguous().view(batch_size, -1, self.d_v * self.num_head)
        if return_weight:
...
class Embedding(nn.Module):
    ...
    def forward(self, input, position_info=True):
        ...
        if position_info:
            ...
            outputs = self.embed(input) * self.embed_scare
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        ...
        if self.normalize_before:
            ...
            out = sublayer(x)
            if isinstance(out, tuple):
                return self.norm(x + self.dropout(out[0])), out[1]
            else:
...
class DecoderCell(nn.Module):
    ...
    def forward(self, input, encoder_outputs, pad_mask, seq_mask):
        ...
        input, weight = self.sublayer[1](input, lambda x: self.cross_attn(x, m, m, pad_mask, True))
        return self.sublayer[2](input, self.FFN), weight
    ...
    def memory_decode(self, input, memory, memory_new, encoder_outputs, src_pad_mask, i):
        ...
        outputs, weight = self.sublayer[1](outputs, lambda x: self.cross_attn(x, m, m, src_pad_mask, True))
        return self.sublayer[2](outputs, self.FFN), memory, weight
...
class Decoder(nn.Module):
    ...
    def forward(self, input, encoder_outputs, pad_mask, seq_mask):
        for decoder_cell in self.decoder:
            input, weight = decoder_cell(input, encoder_outputs, pad_mask, seq_mask)
        if self.normalize_before:
            input = self.layer_norm(input)
        return input, weight.mean(dim=1)

    def generate(self, embed, encoder_outputs, src_pad_mask, memory=None):
        memory_new = None
        for (i, decoder) in enumerate(self.decoder):
            embed, memory_new, weight = decoder.memory_decode(embed, memory, memory_new, encoder_outputs, src_pad_mask, i)
